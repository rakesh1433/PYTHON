SELECT
  ...
  gl_financial_activity.policy_period_start_date AS policy_period_start_date,
  gl_financial_activity.policy_period_end_date AS policy_period_end_date,
  gl_financial_activity.policy_period_public_id,
  gl_financial_activity.policy_period_edit_effective_date AS edit_effective_date,
  gl_financial_activity.job_close_date AS job_close_date,
  gl_financial_activity.accounting_date AS accounting_date,
...
SELECT
  ...
  gl_financial_activity.policy_period_edit_effective_date AS edit_effective_date,
  gl_financial_activity.job_close_date AS job_close_date,
  CAST(CONCAT(CAST(fd.acct_date AS STRING), ' 00:00:00') AS DATETIME) AS accounting_date,
  gl_financial_activity.uw_company_code,
  COALESCE(branch.gl_branch_number, COALESCE(CAST(agnt.branch_number AS STRING), gl_financial_activity.branch_number)) AS branch_number,
...
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      pc_future_dated_transaction_staging.term_number,
      pc_future_dated_transaction_staging.model_number,
